...
folder_path = r'C:\Users\timon\Desktop\1 Uni\1 UCM\AnkiHelperApp'
...
def add_image_tag(row):
    ...
    print(image_tag)
    #if pd.isna(row['Personal Notes']):  # Check if cell in column 'Source' is empty
    if pd.isna(row['Missed Questions']):  # Check if cell in column 'Source' is empty
        row['Missed Questions'] = image_tag  # Add the image tag if empty
        #row['Personal Notes'] = image_tag 
    return row
